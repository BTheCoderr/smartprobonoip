import { WIZARD_STEPS, estimatedMinutesRemaining } from "@/lib/intake/wizardConfig";

interface IntakeWizardProgressProps {
  current: number;
  saveStatus?: string | null;
}

export function IntakeWizardProgress({
  current,
  saveStatus,
}: IntakeWizardProgressProps) {
  const pct = Math.round(((current + 1) / WIZARD_STEPS.length) * 100);
  const minsLeft = estimatedMinutesRemaining(current);
  ...
  return (
    <div className="rounded-md border border-mist-200/90 bg-white p-4 shadow-[var(--shadow-soft)] sm:p-5">
      <div className="mb-3 flex items-start justify-between gap-3">
        <div className="min-w-0">
          <p className="text-[10px] font-mono font-semibold uppercase tracking-[0.14em] text-teal-700">
            Step {current + 1} of {WIZARD_STEPS.length} · ~{minsLeft} min left
          </p>
          <p className="headline-editorial mt-1 text-lg text-navy-900 sm:text-xl">
            {step.label}
          </p>
        </div>
      ...
      <ol className="mt-4 hidden gap-1 sm:grid sm:grid-cols-5">
        {WIZARD_STEPS.map((item, i) => (
          <li
            key={item.id}
