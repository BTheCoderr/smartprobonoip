export const ANALYTICS_EVENTS = [
  "landing_viewed",
  "sample_packet_viewed",
  "sample_pdf_downloaded",
  "pilot_page_viewed",
  "professionals_page_viewed",
  "learn_page_viewed",
  "trust_page_viewed",
  "after_meeting_page_viewed",
  "start_clicked",
  "demo_started",
  "disclaimer_accepted",
  "intake_started",
  "intake_step_viewed",
  "intake_step_completed",
  "intake_validation_error",
  "intake_review_viewed",
  "intake_completed",
  "intake_draft_saved",
  "ownership_step_viewed",
  "ownership_info_completed",
  "ownership_signal_triggered",
  "packet_generated",
  "packet_viewed",
  "pdf_downloaded",
  "attorney_export_completed",
  "clarity_before_recorded",
  "clarity_after_recorded",
  "next_step_viewed",
  "recovery_link_created",
  "recovery_link_copied",
  "recovery_email_requested",
  "recovery_claim_started",
  "recovery_claim_succeeded",
  "recovery_claim_failed",
  "coach_opened",
  "coach_quick_action_clicked",
  "coach_message_sent",
  "coach_response_generated",
  "coach_error",
  "dashboard_viewed",
  "dashboard_filter_changed",
  "csv_exported",
  "resource_link_clicked",
  "similar_reference_link_clicked",
  "feedback_viewed",
  "feedback_submitted",
  "followup_requested",
  "resource_type_selected",
  "research_workspace_viewed",
  "query_copied",
  "external_search_opened",
  "reference_saved",
  "reference_updated",
  "reference_deleted",
  "comparison_helper_used",
  "contact_form_viewed",
  "interest_submitted",
  "partner_interest_clicked",
  "partner_referral_link_clicked",
  "product_walkthrough_viewed",
  "product_walkthrough_step_viewed",
  "product_demo_viewed",
  "product_demo_started",
  "product_demo_completed",
  "pilot_cta_clicked",
  "gap_map_saved",
  "timeline_saved",
  "workspace_viewed",
  "portfolio_sorted",
  "invention_status_changed",
  "recommendation_shown",
  "recommendation_clicked",
  "recommendation_dismissed",
  "recommendation_dismissals_restored",
  "recommendation_feedback_submitted",
  "recommendation_handoff_viewed",
  "recommendation_handoff_confirmed",
  "partner_directory_viewed",
  "partner_filter_applied",
  "partner_search_used",
  "partner_profile_viewed",
  "partner_external_link_clicked",
  "organization_login_viewed",
  "organization_inbox_viewed",
  "organization_referral_viewed",
  "organization_referral_status_changed",
  "organization_referral_shared",
  "organization_metrics_viewed",
  "organization_share_consent_viewed",
  "organization_share_consent_submitted",
] as const;

export type AnalyticsEventName = (typeof ANALYTICS_EVENTS)[number];

const EVENT_SET = new Set<string>(ANALYTICS_EVENTS);

export function isAnalyticsEventName(value: string): value is AnalyticsEventName {
  return EVENT_SET.has(value);
}
