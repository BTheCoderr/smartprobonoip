...
export const PRIVACY_NOTICE = [
  "What we collect: your intake answers, optional clarity ratings (before and after), generated readiness profiles, optional pilot feedback, saved research prep notes you choose to store, per-invention activity milestones (what happened and when — for example \"packet generated\" — not your invention text), document records listing artifacts you generated (title, type, format, and timestamp — not the file itself), and first-party analytics events (event names, routes, signal keys, and safe counts — not your full invention text).",
  ...
  "Partner pilot reporting: authorized partners with a dashboard secret may view aggregated pilot metrics and export CSV summaries. Dashboard API responses and CSV exports omit raw invention descriptions and free-text feedback notes.",
  "Recovery links: if you create a recovery link, we store only a hashed token (never the raw link text). Newly created links are single-use — the first successful claim consumes them. Links issued before this change remain usable until they expire or you revoke them by creating a replacement of the same type. A portfolio link restores every invention created in the same browser session, so it is broader than a single-packet link; creating one does not invalidate an existing packet link.",
  ...
  "Subprocessors: we use Netlify for website hosting and deployment, Supabase for database storage when cloud mode is enabled, optionally OpenAI when an API key is configured for AI-assisted packet generation and the Packet Coach, and optionally Google Tag Manager / Google Analytics 4 on public marketing pages only. These providers process data according to their own terms and only as needed to operate the service.",
